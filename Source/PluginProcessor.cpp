...
void AuxRumbleAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    {
        lowPass[i].prepareToPlay(*crossoverFc, *bandpassQ, sampleRate, AuxPort::IIR::Type::Lowpass);
        highPass[i].prepareToPlay(*crossoverFc, *bandpassQ, sampleRate, AuxPort::IIR::Type::Highpass);
    }
    bandPass.prepareToPlay(*crossoverFc, *bandpassQ, sampleRate, AuxPort::IIR::Type::Bandpass);
...
void AuxRumbleAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (uint32_t i = 0; i < lowPass.size(); i++)
    {
        lowPass[i].prepareToPlay(*crossoverFc, *bandpassQ, this->getSampleRate(), AuxPort::IIR::Type::Lowpass);
        highPass[i].prepareToPlay(*crossoverFc, *bandpassQ, this->getSampleRate(), AuxPort::IIR::Type::Highpass);
        highpassOutput[i] = 0;
    }
